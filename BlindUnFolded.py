...
def blind_main():
    speak("Hello and welcome to BrailleVox Pro! Would you like me to tell you the available options?")
    cmd = listen()
    if "ok"or "yes" in cmd:
        speak('''Option 1. Audio based services. 
              Option 2. Text based services. 
              Option 3. Show log. 
              Option 4. Delete old records from log. 
              Option 5. Exit''')
    ...
    while (True):        
        ...
        elif "option 1" in cmd or "one" in cmd or "audio" in cmd or "voice" in cmd or "audio based" in cmd:
            speak('''These are the options for the audio services. 
                  Option 1. Convert voice to Braille as you speak. 
                  Option 2. Convert an audio file to Braille. 
                  Option 3. Return to main menu.''')
            while (True):
                cmd = listen()
                if "option 1 " in cmd or "one " in cmd or "speak" in cmd or "voice" in cmd:
                    ...
                    break
                elif "option 2 " in cmd or "two" in cmd or "audio file" in cmd or "file" in cmd or "open" in cmd:
                    tk.Tk().withdraw()
                    ...
                    break
                elif "option 3" in cmd or "three" in cmd or "back" in cmd or "return" in cmd or "main menu" in cmd:
                    speak("Waiting for furthur instructions in the main menu.")
                    break
        
        elif "option 2" in cmd or "two" in cmd or "text based" in cmd or "type" in cmd:
            speak('''These are the options for the text services.
                    Option 1. Convert text to Braille by typing. 
                    Option 2. Convert an existing text file to Braille.
                    Option 3. Return to main menu.''')
            
            while (True):
                cmd = listen()
                if "option 1" in cmd or "one" in cmd or "typing" in cmd:
                ...
                elif "option 2" in cmd or "two" in cmd or "existing text" in cmd or "text file" in cmd:
                    tk.Tk().withdraw()
        ...
        elif "option 3" in cmd or "three" in cmd or "show log" in cmd or "log" in cmd:
            decrypt()
            x=open("C:\\ProgramData\\Blind Unfolded\\log.txt",'r')
            data=x.read()
            x.close()
        ...
        elif "option 4" in cmd or "four" in cmd or "delete" in cmd or "remove" in cmd:
...
root = Tk()
root.title("BrailleVox Pro")
root.geometry("1250x625")
image_path = PhotoImage(file = 'C:/Users/Prakhar/Desktop/BLIND UNFOLDED/Blind UnFolded/bg.png')
...
icon = PhotoImage(file = 'C:/Users/Prakhar/Desktop/BLIND UNFOLDED/Blind UnFolded/ok.png')
root.iconphoto(False, icon)

logo = PhotoImage(file='C:/Users/Prakhar/Desktop/BLIND UNFOLDED/Blind UnFolded/logo.png')
logolabel = tk.Label(image=logo)
logolabel.grid(column=0,row=0)
i = tk.Label(root, text="\n\t\t\tVision without sight is now a reality\t\t\t\t", font=("Mongolian Baiti",30))
i.configure(bg= 'gray89') 
i.grid(columnspan=15, column=0, row=1)

frame = Frame(root)
frame.grid(columnspan= 8, column=1, row=0)
textBox = tk.Text(frame, font= (' \Cambria'), height= 20, width= 49, wrap=WORD)
textBox.insert('end', "")
textBox.config(state= "normal")
textBox.focus_set()
textBox.pack()
textBox.configure(bg = 'gray74')
